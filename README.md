...
| File | Role |
| --- | --- |
| `main.c` | The main entry point of the shell that manages the REPL loop and user interaction. |
...
| `path_utils.c` | It searches for the given command in the system's PATH environment variable. It uses getenv() to retrieve the PATH variable and access() to check if the command exists in the directories. |
